...
\texttt{basketball-reference.com} (9 May 2025).  Players with
$<\!20$ games or $<\!30$ 3-point attempts were removed; for multi-team
seasons only the league-aggregate ``TOT'' line was kept, leaving
...
\subsection{Model 1 - Hierarchical Binomial for 3-pt accuracy}

For player $i$ ($i=1,\dots,N$) in position $p_i$:
\[
y_i\mid\theta_i \sim \text{Binomial}(n_i,\theta_i), \qquad
\text{logit}\,\theta_i= \alpha_{p_i}+ \beta\,\text{age\_sc}_i .
\]
Position intercepts follow $\alpha_p\sim\mathcal N(\mu_\alpha,\sigma_\alpha^2)$
with weakly informative hyperpriors
$\mu_\alpha\sim\mathcal N(0,5)$, $\sigma_\alpha\sim\text{Half-Normal}(0,1)$,
and $\beta\sim\mathcal N(0,2)$.

\subsection{Model 2 - Student-$t$ random-slope for PPG}

\[
y_i\sim t_\nu(\mu_i,\sigma),\quad
\mu_i=\alpha_{p_i}+\beta_{p_i}\,\text{age\_sc}_i,
\qquad (\alpha_p,\beta_p)^\top\sim\mathcal N_2(\mathbf0,\Sigma).
\]

\[
\underbrace{
  \alpha_p\sim\mathcal N(\mu_\alpha,\sigma_\alpha^2),\;
  \beta_p\sim\mathcal N(\mu_\beta,\sigma_\beta^2)}_{\text{second level}},\qquad
(\mu_\alpha,\mu_\beta)\sim\mathcal N(\mathbf0,\,10^2I),\;
(\sigma_\alpha,\sigma_\beta)\sim\text{Half-N}(0,2).
\]

Priors for the likelihood scale and degrees of freedom are
$\sigma\sim\text{Half-Normal}(0,10)$ and $\nu\sim\text{Exp}(30^{-1})$; the
correlation matrix of $\Sigma$ follows LKJ$(2)$.
Both models were fitted in \texttt{brms}
(4 chains, 1000 warm-up + 3000 sampling iterations;
\texttt{adapt\_delta = 0.995} removed all divergences).
...
\centering
\csvautotabular[respect underscore=true]{tables/bb_fixed_effects.csv}
\caption{Model 1 fixed effects\label{tab:bbfixed}}
\end{minipage}\hfill
\begin{minipage}{.48\linewidth}
\centering
\csvautotabular[respect underscore=true]{tables/ppg_fixed_effects.csv}
...
\caption{Trace and density overlays for key parameters\label{fig:diagnostics}}
\end{figure}

\begin{figure}[ht]
    \includegraphics[width=.48\linewidth]{ppc_bb.png}\hfill
    \includegraphics[width=.48\linewidth]{ppc_ppg.png}\hfill
    \includegraphics[width=0.5\linewidth]{ppg_resid.png}\hfill
    \caption{Posterior predictive checks.  Simulated means match the observed
    mean within 0.3 SE; Bayesian $p$-values for skewness are 0.46 (left) and 0.52
    (right), indicating no systematic misfit.\label{fig:ppc}}
\end{figure}
